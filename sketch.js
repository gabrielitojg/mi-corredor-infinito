...
function setup() {
  ...
  //radio de colicionar
  corredor.setCollider("circle",0,0,200);
  ...
  carretera = createSprite(width/2,height/2,width + 400,50);
...
function draw() {
  ...
  if(gameState === PLAY){
    ...
    if(touches.length > 0 || keyDown("space")&& corredor.y >= height-400) {
      corredor.velocityY = -8;
      touches = [];
    }
...
function spawnObstacles(){
 if (frameCount % 80 === 0){
   var obstaculo = createSprite(width +10, height-100,20,30 );
